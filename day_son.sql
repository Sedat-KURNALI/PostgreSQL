...
--2. Yol
SELECT name || ' ' || state || ' ' || LENGTH ( state ) + LENGTH( name )  AS name_and_state
FROM workers
ORDER BY LENGTH ( state ) + LENGTH( name );
...
*/
-- salary değeri 3000'den yüksek olan state değerlerini ve 2000'den küçük 
-- olan name değerlerini tekrarsız olarak bulun.
